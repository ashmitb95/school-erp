...
const app = express();
const PORT = process.env.EXAM_SERVICE_PORT || 3005;
...
// Add exam result
app.post('/:id/results', async (req: Request, res: Response) => {
  try {
...
// Get exam results
app.get('/:id/results', async (req: Request, res: Response) => {
  try {
    const { id: exam_id } = req.params;
    const { page, limit } = paginationSchema.parse(req.query);
...
sequelize
  .authenticate()
  .then(() => {
    console.log('Database connection established for exam service');
    app.listen(PORT, () => {
